...
#define RF69_MODE_READY_TIMEOUT_MS 50  // bail out of sendFrame if ModeReady never asserts
#define RF69_TX_TIMEOUT_MS      100    // bail out of sendFrame if PacketSent never asserts
...
class RFM69 {
  public:
    ...
    // Public accessors so application code can detach/reattach the DIO0
    // interrupt around radio.send() (RP2040 SPI-mutex/ISR collision fix).
    uint8_t getInterruptNum() { return _interruptNum; }
    void attachIsr() { attachInterrupt(_interruptNum, RFM69::isr0, RISING); }
    void detachIsr() { detachInterrupt(_interruptNum); }
    // True if the most recent sendFrame saw PacketSent assert before the
    // timeout. Must be checked INSTEAD of polling IRQ2 afterwards, because
    // the PacketSent flag auto-clears when the chip leaves TX mode.
    bool lastTxOk = false;
