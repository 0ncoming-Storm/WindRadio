...
bool RFM69::initialize(uint8_t freqBand, uint16_t nodeID, uint8_t networkID) {
  ...
  const uint8_t CONFIG[][2] = {
    ...
    /* 0x07 */ { REG_FRFMSB, (uint8_t) (freqBand==RF69_315MHZ ? RF_FRFMSB_315 : (freqBand==RF69_433MHZ ? RF_FRFMSB_433_92 : (freqBand==RF69_868MHZ ? RF_FRFMSB_868 : RF_FRFMSB_915))) },
    /* 0x08 */ { REG_FRFMID, (uint8_t) (freqBand==RF69_315MHZ ? RF_FRFMID_315 : (freqBand==RF69_433MHZ ? RF_FRFMID_433_92 : (freqBand==RF69_868MHZ ? RF_FRFMID_868 : RF_FRFMID_915))) },
    /* 0x09 */ { REG_FRFLSB, (uint8_t) (freqBand==RF69_315MHZ ? RF_FRFLSB_315 : (freqBand==RF69_433MHZ ? RF_FRFLSB_433_92 : (freqBand==RF69_868MHZ ? RF_FRFLSB_868 : RF_FRFLSB_915))) },
...
void RFM69::sendFrame(uint16_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK) {
  ...
  while ((readReg(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00) {
    if (millis() - mrStart > RF69_MODE_READY_TIMEOUT_MS) return; // radio wedged - bail instead of hanging
  }
  ...
  while ((readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT) == 0x00) {
    if (millis() - psStart > RF69_TX_TIMEOUT_MS) break; // TX never completed - bail instead of hanging
  }
  lastTxOk = (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT) != 0x00;
  setMode(RF69_MODE_STANDBY);
}
...
int16_t RFM69::readRSSI(bool forceTrigger) {
  ...
  if (forceTrigger) {
    ...
    uint32_t rssiStart = millis();
    while ((readReg(REG_RSSICONFIG) & RF_RSSI_DONE) == 0x00) {
      if (millis() - rssiStart > 10) break; // RSSI never ready - bail instead of hanging
    }
  }
